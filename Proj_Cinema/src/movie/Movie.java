...
//5. 영화 객체(제목,장르)를 읽어(movies.txt)와 List 만들기
public class Movie {
...
//영화 객체 정보를 파일 출력 형식으로 변환
    private String toFileString() { // 객체 정보를 문자열로 변환
        return String.format("%d,%s,%s", id, title, genre);
    }
//영화정보기능 삭제 - 전달된 대표값을 movies.txt파일에서 찾아 해당 행 삭제
...
//영화 조회 - 영화를 movies.txt파일에서 찾아 Movie객체로 반환
